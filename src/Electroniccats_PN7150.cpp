...
Electroniccats_PN7150::Electroniccats_PN7150(
    uint8_t IRQpin,
    uint8_t VENpin,
    uint8_t I2Caddress,
    ChipModel chipModel,
    ...
    if(_VENpin != 255) pinMode(_VENpin, OUTPUT);
...
bool Electroniccats_PN7150::getMessage(
    ...
    while(!isTimeOut()) {
        ...
        if(rxMessageLength)
            break;
        else if(timeout == TIMEOUT_INFINITE_MARKER)
            setTimeOut(timeout);
    }
...
bool Electroniccats_PN7150::hasMessage() const {
    return (
        HIGH == digitalRead(_IRQpin)); // PN7150 indicates it has data by driving IRQ signal HIGH
}
...
#ifdef DEBUG2
                Serial.println(
                    "[DEBUG] payload[" + String(index) + "] = 0x" + String(rxBuffer[index], HEX));
#endif
                index++;
            }
            index = 0;
...
int Electroniccats_PN7150::getFirmwareVersion() {
    return ((gNfcController_fw_version[0] & 0xFF) << 16) |
           ((gNfcController_fw_version[1] & 0xFF) << 8) | (gNfcController_fw_version[2] & 0xFF);
}
...
uint8_t Electroniccats_PN7150::ConfigMode(uint8_t modeSE) {
    ...
    // RW Mode
    const uint8_t DM_RW[] = {
        0x1, 0x1, 0x1, 0x2, 0x1, 0x1, 0x3, 0x1, 0x1, 0x4, 0x1, 0x2, 0x80, 0x01, 0x80};
    uint8_t NCIPropAct[] = {0x2F, 0x02, 0x00};

    // P2P Support
    const uint8_t DM_P2P[] = {0x5, 0x3, 0x3};
    const uint8_t R_P2P[] = {0x1, 0x3, 0x0, 0x1, 0x5};
    uint8_t NCISetConfig_NFC[] = {0x20, 0x02, 0x1F, 0x02, 0x29, 0x0D, 0x46, 0x66, 0x6D,
                                  0x01, 0x01, 0x11, 0x03, 0x02, 0x00, 0x01, 0x04, 0x01,
                                  0xFA, 0x61, 0x0D, 0x46, 0x66, 0x6D, 0x01, 0x01, 0x11,
                                  0x03, 0x02, 0x00, 0x01, 0x04, 0x01, 0xFA};
...
bool Electroniccats_PN7150::configureSettings(void) {
   ...
   */
    uint8_t NxpNci_CORE_CONF_EXTN[] = {
        ...
        0x02,
        0x0D,
        0x03, /* CORE_SET_CONFIG_CMD */
        0xA0,
        0x40,
        0x01,
        0x00, /* TAG_DETECTOR_CFG */
        0xA0,
        0x41,
        0x01,
        0x04, /* TAG_DETECTOR_THRESHOLD_CFG */
        0xA0,
        0x43,
        0x01,
        0x00 /* TAG_DETECTOR_FALLBACK_CNT_CFG */
    };
   ...
   * PN7150)*/
    ...
    uint8_t NxpNci_TVDD_CONF_2ndGen[] = {
        0x20, 0x02, 0x07, 0x01, 0xA0, 0x0E, 0x03, 0x02, 0x09, 0x00};
    uint8_t NxpNci_TVDD_CONF_3rdGen[] = {
        0x20,
        0x02,
        0x0F,
        0x01,
   ...
   * Controller demo kit */
    uint8_t NxpNci_RF_CONF_3rdGen[] = {0x20, 0x02, 0x4C, 0x09, 0xA0, 0x0D, 0x03, 0x78, 0x0D, 0x02,
                                       0xA0, 0x0D, 0x03, 0x78, 0x14, 0x02, 0xA0, 0x0D, 0x06, 0x4C,
                                       0x44, 0x65, 0x09, 0x00, 0x00, 0xA0, 0x0D, 0x06, 0x4C, 0x2D,
                                       0x05, 0x35, 0x1E, 0x01, 0xA0, 0x0D, 0x06, 0x82, 0x4A, 0x55,
                                       0x07, 0x00, 0x07, 0xA0, 0x0D, 0x06, 0x44, 0x44, 0x03, 0x04,
                                       0xC4, 0x00, 0xA0, 0x0D, 0x06, 0x46, 0x30, 0x50, 0x00, 0x18,
                                       0x00, 0xA0, 0x0D, 0x06, 0x48, 0x30, 0x50, 0x00, 0x18, 0x00,
  ...
  else
  {
    ...
    if(sizeof(NxpNci_CORE_CONF_EXTN) != 0) {
        if(_chipModel == PN7150)
            ...
            (void)writeData(NxpNci_CORE_CONF_EXTN_3rdGen, sizeof(NxpNci_CORE_CONF_EXTN_3rdGen));
...
wait:
    do {
        getFlag = getMessage(tout > 0 ? tout : 1337); // Infinite loop, waiting for response
    ...
    if(rxBuffer[1] == 0x05) {
        ...
        do {
            if(!getMessage(100)) return ERROR;
...
bool Electroniccats_PN7150::cardModeSend(unsigned char* pData, unsigned char DataSize) {
    ...
    return (writeData(Cmd, DataSize + 3) == SUCCESS);
}
...
void Electroniccats_PN7150::processP2pMode(RfIntf_t RfIntf) {
    ...
    status = ERROR;
    getMessage(2000);
    if(rxMessageLength > 0) status = SUCCESS;
...
void Electroniccats_PN7150::presenceCheck(RfIntf_t RfIntf) {
    ...
    uint8_t NCIPresCheckIso15693[] = {
        0x00, 0x00, 0x0B, 0x26, 0x01, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    ...
    case PROT_ISODEP:
        do {
            ...
            getMessage();
            getMessage(100);
        } while((rxBuffer[0] == 0x6F) && (rxBuffer[1] == 0x11) && (rxBuffer[2] == 0x01) &&
                (rxBuffer[3] == 0x01));
        break;
    ...
    case PROT_ISO15693:
        ...
        } while((status == SUCCESS) && (rxBuffer[0] == 0x00) && (rxBuffer[1] == 0x00) &&
                (rxBuffer[rxMessageLength - 1] == 0x00));
        break;
...
bool Electroniccats_PN7150::readerTagCmd(
    ...
    if((rxBuffer[0] == 0x0) && (rxBuffer[1] == 0x0)) status = SUCCESS;
...
bool Electroniccats_PN7150::NxpNci_FactoryTest_Prbs(
