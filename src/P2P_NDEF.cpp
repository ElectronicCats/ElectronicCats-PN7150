...
static void
    ParseLlcp(unsigned char* pBuf, unsigned short BufSize, P2P_NDEF_LlcpHeader_t* pLlcpHeader) {
    uint8_t i = 2;
    ...
    while(i < BufSize) {
        ...
        case SN:
            memcpy(
                pLlcpHeader->Sn,
                &pBuf[i + 2],
                pBuf[i + 1] < sizeof(pLlcpHeader->Sn) ? pBuf[i + 1] : sizeof(pLlcpHeader->Sn));
            break;
        default:
            break;
        }
...
void P2P_NDEF_Next(
    ...
    *pRsp_size = 0;
    ...
    case CC:
        /* Connection to remote SNEP server completed, send NDEF message inside SNEP
     * PUT request */
        eP2P_SnepClient_State = SnepClientConnected;
        break;
