...
const unsigned char T4T_NDEF_EMU_APP_Select[] =
    {0x00, 0xA4, 0x04, 0x00, 0x07, 0xD2, 0x76, 0x00, 0x00, 0x85, 0x01, 0x01, 0x00};
const unsigned char T4T_NDEF_EMU_CC[] =
...
static void
    T4T_NDEF_EMU_FillRsp(unsigned char* pRsp, unsigned short offset, unsigned char length) {
    if(offset == 0) {
    ...
    if((offset + length) >= (T4T_NdefMessage_length + 2)) {
        ...
        if(ndefSendCallback != NULL) ndefSendCallback();
    }
}
