...
void RW_NDEF_MIFARE_Read_Next(
    unsigned char* pRsp,
    unsigned short Rsp_size,
    unsigned char* pCmd,
    unsigned short* pCmd_size) {
    ...
    case Reading_FirstBlk:
       ...
       * the application and stop reading */
            ...
            if(RW_NDEF_MIFARE_Ndef.MessageSize <= ((Rsp_size - 1) - Tmp - 2)) {
                memcpy(
                    RW_NDEF_MIFARE_Ndef.pMessage, &pRsp[Tmp + 2], RW_NDEF_MIFARE_Ndef.MessageSize);
                ...
                // Run custom callbacks
                if(updateNdefMessageCallback != NULL) {
                    updateNdefMessageCallback(
                        RW_NDEF_MIFARE_Ndef.pMessage, RW_NDEF_MIFARE_Ndef.MessageSize);
                }
                ...
                memcpy(
                    RW_NDEF_MIFARE_Ndef.pMessage, &pRsp[Tmp + 2], RW_NDEF_MIFARE_Ndef.MessagePtr);
                RW_NDEF_MIFARE_Ndef.BlkNb++;
    ...
    case Reading_Data:
        if((Rsp_size == 18) && (pRsp[Rsp_size - 1] == 0x00)) {
            ...
            if((RW_NDEF_MIFARE_Ndef.MessageSize - RW_NDEF_MIFARE_Ndef.MessagePtr) < 16) {
                memcpy(
                    &RW_NDEF_MIFARE_Ndef.pMessage[RW_NDEF_MIFARE_Ndef.MessagePtr],
                    pRsp + 1,
                    RW_NDEF_MIFARE_Ndef.MessageSize - RW_NDEF_MIFARE_Ndef.MessagePtr);
                ...
                memcpy(
                    &RW_NDEF_MIFARE_Ndef.pMessage[RW_NDEF_MIFARE_Ndef.MessagePtr], pRsp + 1, 16);
...
void RW_NDEF_MIFARE_Write_Next(
    unsigned char* pRsp,
    unsigned short Rsp_size,
    unsigned char* pCmd,
    unsigned short* pCmd_size) {
