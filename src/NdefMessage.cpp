...
unsigned char* NdefMessage::content = NULL;
unsigned short NdefMessage::contentLength = 0;
unsigned char* NdefMessage::newContent = NULL;
unsigned short NdefMessage::newContentLength = 0;
uint8_t NdefMessage::recordCounter = 0;
...
void NdefMessage::updateHeaderFlags() {
    // Use fixed size arrays instead of VLA for C++ compatibility
    uint8_t headersPositions[MAX_NDEF_RECORDS];
    uint8_t previousHeaders[MAX_NDEF_RECORDS];
...
void NdefMessage::addRecord(NdefRecord record) {
    ...
    if(NdefMessage::newContent != NULL) {
        delete[] NdefMessage::newContent;
    }
...
void NdefMessage::addMimeMediaRecord(
    String mimeType,
    const char* payload,
...
void NdefMessage::addWiFiRecord(
    String ssid,
    ...
    unsigned char* payload =
        new unsigned char[ssid.length() + password.length() + WIFI_RECORD_HEADER_SIZE];
    ...
    record.setHeaderFlags(NDEF_HEADER_FLAGS_SINGLE_MEDIA_RECORD);
    record.setTypeLength(mimeType.length());
    size_t payloadSize = ssid.length() + password.length() + 29;
    record.setPayloadLength(payloadSize);
    record.setRecordType(mimeType);
    record.setPayload((const char*)payload, payloadSize);
