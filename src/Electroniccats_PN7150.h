...
private:
    ...
    uint8_t rxBuffer[MaxPayloadSize + MsgHeaderSize]; // buffer where we store bytes received until
        // they form a complete message
    unsigned long timeOut;
    unsigned long timeOutStartTime;
    uint32_t rxMessageLength; // length of the last message received. As these are not
        // 0x00 terminated, we need to remember the length
    uint8_t gNfcController_generation = 0;
    uint8_t gNfcController_fw_version[3] = {0};
    void setTimeOut(unsigned long); // set a timeOut for an expected next event, eg
        // reception of Response after sending a Command
    bool isTimeOut() const;
    uint8_t wakeupNCI();
    bool getMessage(
        uint16_t timeout = 5); // 5 miliseconds as default to wait for interrupt responses
...
public:
    ...
    uint32_t
        readData(uint8_t data[]) const; // read data from PN7150, returns the amount of bytes read
    ...
    void processReaderMode(
        RfIntf_t RfIntf,
        RW_Operation_t Operation); // Deprecated, use waitForTagRemoval(), readNdefMessage()
