...
String NdefRecord::getWiFiSSID() {
    ...
    for(unsigned int i = 0; i < payloadLength; i++) {
        if(payload[i] == 0x45 && (i + 3) < payloadLength) {
...
String NdefRecord::getUri() {
    ...
    unsigned char save = payload[payloadLength];
    payload[payloadLength] = '\0';
    String prefix = String(ndef_helper_UriHead(payload[0]));
    uri = prefix + String(reinterpret_cast<const char*>(&payload[1]));
    payload[payloadLength] = save;
...
void NdefRecord::setPayload(String payload) {
    ...
    this->payload = new unsigned char[payload.length() + 1];
    strncpy((char*)this->payload, payload.c_str(), payload.length() + 1);
...
void NdefRecord::setRecordType(String type) {
    this->mimeMediaType = new unsigned char[type.length() + 1];
    strncpy((char*)this->mimeMediaType, type.c_str(), type.length() + 1);
}
...
void NdefRecord::setLanguageCode(String languageCode) {
    ...
    this->languageCode = new unsigned char[languageCode.length() + 1];
    strncpy((char*)this->languageCode, languageCode.c_str(), languageCode.length() + 1);
}
